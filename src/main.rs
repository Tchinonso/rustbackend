...
// POST /todos
async fn add_todos(State(data): State<AppState>, Json(item): Json<CreateTodoItem>) -> impl IntoResponse {
...
async fn main() {
    ...
    // Bind to port provided by Render or fallback to 8080
    let port = std::env::var("PORT").unwrap_or_else(|_| "8080".to_string());
    let addr: SocketAddr = format!("0.0.0.0:{}", port).parse().expect("invalid address");
    println!("🚀 Server running on http://{}", addr);

    // Correct way to run the Axum server
...
//     // ✅ Bind to PORT provided by Render, or fallback to 8080 locally
//     let port = std::env::var("PORT").unwrap_or_else(|_| "8080".to_string());
